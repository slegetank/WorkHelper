...
if __name__ == '__main__':
    if not os.path.exists('%s/symbolicatecrash' % os.path.expanduser('~')):
        ...
        if len(cmdPath) == 0:
            print 'Found no symbolicatecrash'
            sys.exit(-1)
        else:
            shutil.copyfile(cmdPath, '%s/symbolicatecrash' % os.path.expanduser('~'))
            os.system('chmod +x %s' % '%s/symbolicatecrash' % os.path.expanduser('~'))
    ...
    for logFile in os.listdir(logDir):
        if logFile.startswith(appName) and (logFile.endswith('.ips') or logFile.endswith('.crash')):
            count += 1
            os.system('%s/symbolicatecrash \'%s\' \'%s\' > \'%s/%s%d.log\'' % (os.path.expanduser('~'), os.path.join(logDir, logFile), symbolFile, logDir, appName, count))
